 ...
 *      ブラウザで手動追跡する場合は JRAトップ →「出馬表」と進んでも同じ画面に到達できる。
 *   ページ名: 「JRA 日本中央競馬会」公式トップページ
 *
 * ▼ STEP B ── 「出馬表」リンクをクリック → 開催選択ページへ
 *   操作: JRAデータベース（accessS.html）の左メニューから
 *          onclick に "accessD" を含む「出馬表」リンクをクリック
 *   変化: 出馬表の開催選択ページ（accessD.html）へ遷移する。
 *          「2回福島5日」「1回小倉1日」のような開催リンクが並ぶ。
 *          ※ 過去レース結果とは異なり、年月セレクトは不要（開催中・直近が自動表示される）
 *
 * ▼ STEP C ── 開催一覧を収集（操作なし）
 *   操作: なし（ページを読み取るだけ）
 *   収集方法:
 *     onclick に "pw01drl" を含むリンクから「X回XX Y日」形式のテキストを抽出する。
 *     pw01drl コードの末尾8桁（substring(17, 25)）が YYYYMMDD 形式の開催日。
 *     例: "pw01drl00032026020520260711" → 開催日 2026-07-11
 *   特記:
 *     ・「2回福島5日馬番確定」のように "馬番確定" が付く表記は除去して正規化する
 *     ・同じ開催名の重複リンクは除去する
 *
 * ▼ STEP D ── 絞り込み / --list-only 判定
 *   優先順位: --kaisai 指定 ＞ --date 指定 ＞ 全件
 *   ・--kaisai 指定 → その開催名のみを対象にする
 *   ・--date 指定   → 指定日付（例: 2026-07-12）の開催のみを対象にする
 *   ・--list-only   → 絞り込み後の開催名・日付一覧を JSON で出力してここで終了
 *
 * ▼ STEP E ── 各開催について外側ループ（対象開催の数だけ繰り返す）
 *
 *   ┌─ E-1: 開催選択ページへ戻り、開催ボタンをクリック
 *   │   操作:
 *   │     毎回 accessS.html →「出馬表」→ 開催選択ページへ戻ってから
 *   │     「2回福島5日」等の開催リンク（onclick に "pw01drl" を含む）をクリック
 *   │     ※ 毎回やり直す理由: ページ遷移後に開催一覧が消えるため、安定したスタート地点を確保する
 *   │   変化: その開催のレース一覧ページへ遷移する（table#race_list が表示される）
 *   │
 *   ├─ E-2: 出馬表リンクを全レース分まとめて収集（操作なし）
 *   │   収集対象: a[href*="CNAME=pw01dde"] 形式のリンク
 *   │   CNAMEコード解析:
 *   │     "pw01dde01" + 場所(2桁) + 年(4桁) + 回(2桁) + 日次(2桁) + レース番号(2桁) + 日付(8桁)
 *   │     例: "pw01dde0103202602050120260711"
 *   │         → substring(19, 21) = レース番号（01 → 1R）
 *   │         → substring(21, 29) = 日付（20260711 → 2026-07-11）
 *   │   CNAME重複除去・レース番号の昇順ソートをしてから処理する
 *   │
 *   └─ E-3: 各レースについて内側ループ（レース数だけ繰り返す）
 *
 *       ┌─ F-1: 出馬表ページへ直接 goto
 *       │   操作: page.goto(href) で出馬表ページへ直接遷移する
 *       │          ※ 他スクリプトと違い「戻る（goBack）」は不要。
 *       │             各レースの URL が独立しているため直接 goto で遷移できる。
 *       │   変化: その出馬表ページが開く（table.basic に馬情報が並ぶ）
 *       │
 *       └─ F-2: 出馬表テーブルをパース（操作なし）
 *           対象要素: table.basic > tbody > tr（1行＝1頭）
 *             td.waku img[alt="枠N白/黒"] ← 枠番（alt テキストから数字を抽出）
 *             td.num                       ← 馬番
 *             td.horse
 *               .name a                   ← 馬名
 *               .result_line .result      ← 戦績（例: "(10-3-2-5)"）
 *               .result_line .win         ← 総賞金（title 属性から取得）
 *               .owner                    ← 馬主
 *               .breeder                  ← 生産者
 *               a[onclick*=accessC]       ← 調教師名
 *               .division                 ← 所属（美浦・栗東等）
 *               .family_line .sire        ← 父名
 *               .family_line .mare        ← 母名
 *               .bloodmare                ← 母の父名
 *             td.jockey
 *               a[onclick*=accessK]       ← 騎手名
 *               .age                      ← 性齢（例: "牡3"）
 *               .weight                   ← 負担重量
 *             td.past.p1〜p4              ← 前走〜4走前（各走ごとに以下を取得）
 *               .date_line .date          ← 日付
 *               .date_line .rc            ← 競馬場
 *               .race_line .name a        ← レース名（href から過去レース番号も抽出）
 *               .r_class img[alt]         ← グレード
 *               .place_line .place        ← 着順
 *               .place_line .num .max     ← 頭数
 *               .place_line .num .gate    ← 枠番
 *               .place_line .num .pop     ← 人気
 *               .info_line1 .jockey       ← 騎手
 *               .info_line1 .weight       ← 負担重量
 *               .info_line2 .dist         ← 距離
 *               .info_line2 .time         ← タイム
 *               .info_line2 .condition    ← 馬場状態
 *               .info_line2 .h_weight     ← 馬体重
 *               .corner_list li           ← コーナー通過順（1〜4コーナー）
 *               .info_line3 .f3           ← 上がり3F
 *               .info_line3 .fin          ← 1着馬名・タイム差
 *
 * ▼ 全開催ループ終了後 ── JSON を stdout に出力
 *   kaisai 指定1件 → { kaisai, date, kaisuu, basho, basho_code, day, races }
 *   date or 全件  → { date, data: [ 開催1, 開催2, ... ] }
 *
 * ════════════════════════════════════════════════════════════════
 *
 * 【引数】
