 ...
 *   画面の説明:
 *     JRA公式サイトのトップ。ヘッダーに「レース情報」「競馬場案内」等の
 *     グローバルメニューが並んでいる。
 *
 * ▼ STEP A-2 ── 「レース結果」メニューをクリック
 *   操作:
 *     グローバルメニュー内にある「レース結果」リンクをクリック
 *     ※ このリンクの onclick 属性に "accessS.html','pw01sli00" という
 *       文字列が含まれているものをスクリプトは探している
 *   変化:
 *     開催一覧ページに切り替わる（フレーム内遷移の場合あり）。
 *     直近の開催名がリンク形式で並ぶ一覧が表示される。
 *
 * ▼ STEP B ── 開催一覧から対象を最大6件収集
 *   操作: なし（ページを読み取るだけ）
 *   画面の説明:
 *     「1回東京1日」「1回中山1日」のような形式の開催リンクが
 *     複数並んでいる。スクリプトは先頭6件のみを処理対象とする。
 *     7件目以降の古い開催は無視される。
 *
 * ▼ STEP C ── 各開催について以下を繰り返す（最大6回ループ）
 *
 *   ┌─ C-1: 開催リンクをクリック
 *   │   操作: 「1回東京1日」等の開催リンクをクリック
 *   │   変化: その開催のレース一覧ページへ遷移する。
 *   │          最初は 1R だけが表示されていることが多い。
 *   │
 *   ├─ C-2: 「全てのレースを表示」ボタンをクリック
 *   │   操作: ページ内にある「全てのレースを表示」リンクをクリック
 *   │          ※ ボタンが存在しない場合（既に全件表示中）はスキップして次の開催へ
 *   │   変化: 1R〜最終Rまでの全レース結果テーブルが1ページに展開される。
 *   │
 *   ├─ C-3: 全レース結果をパース（ブラウザ操作なし）
 *   │   操作: なし（JavaScriptでDOMを読み取るだけ）
 *   │   取得方法:
 *   │     ① レース番号は「印刷用ページ」リンクの onclick 属性から取得
 *   │        例: onclick に "pw01spr20230115010501..." → 末尾のレース番号2桁を抽出
 *   │        ※ テーブルの tr 要素にはレース番号が直接書かれていないため、
 *   │           この方法でレース番号リストを先に作っておく
 *   │     ② tbody の全 tr を上から順に読む
 *   │        ・着順が 1〜28 の整数の行のみ有効（「除外」「中止」等は除外）
 *   │        ・着順=1 が2回目に出現したら → 次のレースに切り替わったと判断
 *   │     ③ 各行から [着順] [馬番(td[2])] [馬名(td[3])] [騎手名] を取得
 *   │        ・騎手名は accessK.html へのリンクのテキストから取得
 *   │
 *   └─ C-4: 開催一覧ページへ戻る
 *       操作: onclick に "pw01sli00" を含む「開催一覧」リンクをクリック
 *       変化: STEP B の開催一覧ページへ戻る → 次の開催の C-1 へ進む
 *
 * ▼ 全開催ループ終了後 ── JSON を stdout に出力
 *   全開催・全レース分の着順データをまとめて JSON として出力して終了する。
 *
 * ════════════════════════════════════════════════════════════════
 *
 * 【設計上の注意】
