 ...
 *
 * ▼ STEP E ── 各開催についてループ（対象開催の数だけ繰り返す）
 *
 ...
 *   │   変化: その開催のレース一覧ページへ遷移する（最初は 1R のみ表示のことが多い）
 *   │
 *   ├─ E-2: 「全てのレースを表示」ボタンをクリック
 *   │   操作: ページ内の「全てのレースを表示」リンクをクリック
 *   │          ※ ボタンがない場合（既に全件表示中）はスキップして続行
 *   │   変化: 1R〜最終Rまでの全レース払戻テーブルが1ページに展開される
 *   │
 *   └─ E-3: 払戻金データをパース（ブラウザ操作なし）
 *       操作: なし（JavaScriptでDOMを読み取るだけ）
 *       対象要素:
 *         div[id^="race_result_"]   ← レース単位ブロック（id例: race_result_5R）
 *           h2 .race_name           ← レース名
 *           li.win                  ← 単勝
 *           li.place                ← 複勝
 *           li.wakuren              ← 枠連
 ...
 *               div.pop             ← 人気順位（例: "2人気"）
 *       ※ 日付は h1 タグのテキスト「YYYY年MM月DD日」から取得する
 *
