 ...
 * ════════════════════════════════════════════════════════════════
 *
 * ▼ STEP A ── JRAデータベース トップへアクセス
 *   URL: https://www.jra.go.jp/JRADB/accessS.html
 *   ※ ブラウザで手動追跡する場合は https://www.jra.go.jp/ から
 *      「レース結果」→「過去のレース結果」と進んでも同じ画面に到達できる
 *   ページ名: 「JRAデータベース」
 *   画面の説明:
 *     グレー背景のシンプルなトップページ。
 *     左カラムに「出走表」「過去のレース結果」「地方競馬」などの
 *     メニューリンクが縦に並んでいる。
 *
 * ▼ STEP B ── 「過去のレース結果」リンクをクリック
 *   操作: 左メニューの「過去のレース結果」というテキストリンクをクリック
 *   変化:
 *     同ページの右エリア（またはフレーム内）に
 *     「年」「月」のセレクトボックスと「検索」ボタンが表示される。
 *     ※ ページ遷移ではなくフレームの中身が切り替わる
 *
 * ▼ STEP C ── 年・月を選択して検索ボタンをクリック
 *   操作:
 *     (1) 1番目の <select> で「年」を選ぶ（例: 2023）
 *     (2) 2番目の <select> で「月」を選ぶ（例: 01）
 *     (3) onclick="getSelectData();" が付いたリンク（検索ボタン）をクリック
 *   変化:
 *     指定した年月に開催された競馬場・回・日の一覧が
 *     リンク形式で表示される（例: 1回中山1日 / 1回京都1日 …）
 *
 * ▼ STEP D ── 開催名ボタンをクリック（例: 「1回中山1日」）
 *   操作:
 *     引数 --kaisai で指定した開催名と一致するリンクをクリック
 *     （例: テキストが「1回中山1日」のリンク）
 *   変化:
 *     その開催のレース一覧ページに移動する。
 *     最初は 1R のみが表示されている状態のことが多い。
 *
 * ▼ STEP E ── 「全てのレースを表示」ボタンをクリック
 *   操作:
 *     ページ内に「全てのレースを表示」というリンクまたはボタンがあればクリック
 *     ※ 既に全レース表示済みの場合はこのボタンが存在しないため、スキップして続行
 *   変化:
 *     1R〜最終Rまでの全レース結果テーブルが1ページに展開される。
 *     各レースは「div.race_result_unit」というブロックで区切られて並ぶ。
 *
 * ▼ STEP F ── 全レース結果テーブルを一括パース（ブラウザ操作なし）
 *   操作: なし（JavaScriptでDOMを読み取るだけ）
 *   対象要素:
 *     ・div.race_result_unit[id="race_result_NR"]  ← レース単位ブロック（N=レース番号）
 *     ・table.basic.narrow-xy.striped              ← 各レースの着順テーブル
 *     ・tbody tr                                   ← 1行＝1頭
 *       └─ td[0] 着順 / td[1] 枠 / td[2] 馬番 / td[3] 馬名
 *   取得データ:
 *     着順（数字 or「中止」「除外」等の文字列）・馬番・馬名
 *
 * ════════════════════════════════════════════════════════════════
 *
 * 【使い方】
